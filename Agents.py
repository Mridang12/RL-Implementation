...
class DQNAgent():
    ...
    def __init__(self,layerSizes, epsilon, eps_decay, min_eps, mem_size, batch_size, 
    discount_fact, update_freq, target_update_freq, lr, num_actions, 
    start_training_after, LR_decay_rate = 1):
        ...
        self.LR_decayer = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.optimizer, gamma=LR_decay_rate)
...
class DoubleDQNAgent(DQNAgent):
    ...
    discount_fact, update_freq, target_update_freq, lr, num_actions, 
    start_training_after, LR_decay_rate = 1):
        super().__init__(layerSizes, epsilon, eps_decay, min_eps, mem_size, batch_size, 
    discount_fact, update_freq, target_update_freq, lr, num_actions, 
    start_training_after, LR_decay_rate)
    ...
    def train(self):
        ...
        with torch.no_grad():
            pred_actions_for_next_state = self.policyNet(next_states).argmax(dim=1).unsqueeze(1)


        with torch.no_grad():
            #Instead of calculating max from tnet, use tnet values for pred actions from pnet.
            target_Qvals = self.targetNet(next_states).gather(1, pred_actions_for_next_state)
